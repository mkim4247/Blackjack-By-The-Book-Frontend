import { assignHandValue, getCountFromHand } from "./helperMethods";

const RAILS_API = "http://localhost:4247/api/v1/";
const HEROKU_API =
  "https://blackjack-by-the-book-backend.herokuapp.com/api/v1/";
...
const checkPlayerBlackJack = () => {
  return (dispatch, getStore) => {
    ...
    if (playerHand.find((card) => card.value === "ACE") && playerScore === 21) {
      if (
        dealerHand.find((card) => card.value === "ACE") &&
        dealerScore === 21
      ) {
...
const checkDealerFaceDown = () => {
  return (dispatch, getStore) => {
    ...
    if (dealerHand[1].value === "ACE" && dealerScore === 21) {
      if (
        playerHand.find((card) => card.value === "ACE") &&
        playerHand.score === 21
      ) {
        dispatch(playerPush(hand));
      } else {
  ...
  return (dispatch, getStore) => {
    ...
    fetch(DOC_API + `${deckId}/draw/?count=2`)
      ...
      .then((deck) => {
        ...
        let score = [
          assignHandValue(splitHand1).score,
          assignHandValue(splitHand2).score,
        ];
