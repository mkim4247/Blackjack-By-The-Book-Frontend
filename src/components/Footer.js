...
const Footer = (props) => {
  ...
  const checkTable = () => {
    ...
    if (currentHand.cards[0].value === currentHand.cards[1].value) {
      ...
      currentHand.cards[0].value === "ACE" ||
      currentHand.cards[1].value === "ACE"
    ) {
  ...
  return (
    <div className="footer">
      <div
        className="left"
        style={props.bet > 0 ? { color: "lightGreen" } : { color: "yellow" }}
      >
        Bet Size: {props.bet}
      </div>
      <div className="left">
        {props.showStrategy ? "Count: " + this.props.count : null}
      </div>
      <div className="left">
        {props.showStrategy
      ...
      <button className="right" onClick={props.toggleStrategy}>
        {props.showStrategy ? "Hide Strategy" : "Show Strategy"}
      </button>
    </div>
  );
};
